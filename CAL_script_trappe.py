...
folder = f'cal_{H:.1f}_{frac:.1f}_{HEIGHT:.1f}_trappe'
dir = 'ff/trappe'
...
for sc in [1]:
    with open(f'na_diff/{folder}/run.sh', 'w') as f:
        f.write(f'#!/bin/bash\n#SBATCH --job-name=gromacs_{filename}\n#SBATCH --partition=RT\n#SBATCH --nodes=3\n#SBATCH --ntasks-per-node=16\n\n')
        f.write(f'srun -n 1 gmx_mpi grompp -f ../nvt_cal_steep.mdp -c {filename}_init.gro -p trappe.top -o {filename} -maxwarn 10\n')
        f.write(f'srun -n 16 gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename}\n')
        f.write(f'rm ./#*#')
        f.write(f'rm *pdb')
        f.write(f'srun -n 1 gmx_mpi grompp -f ../nvt_cal_short.mdp -c {filename}.gro -p trappe.top -o {filename} -maxwarn 10\n')
        f.write(f'srun -n 48 gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -dlb yes\n')
        f.write(f'rm ./#*#')
        f.write(f'srun -n 1 gmx_mpi grompp -f ../nvt_cal_run.mdp -c {filename}.gro -p trappe.top -o {filename} -maxwarn 10\n')
        f.write(f'srun -n 48 gmx_mpi mdrun -s -o -x -c -e -g -v -deffnm {filename} -dlb yes\n')
        f.write(f'rm ./#*#')
