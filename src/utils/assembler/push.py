...
def push_atoms_apart(structure, min_distance2, max_distance2, iteration_lim=10):
    ...
    while (overlap):
        ...
        for i, atom in enumerate(tqdm(new_structure.atoms, desc='Atoms')):
            for j, other_atom in enumerate(new_structure.atoms):
                if j > i and atom.mol_idx != other_atom.mol_idx:
